...
pub fn get_all_modules_cascade_aux(module_path: &Path, seen_paths: &mut HashSet<String>, modules: &mut HashMap<(String, String), ModuleInfo>, file_cache: &mut FileCache) -> Result<(), NessaError> {
    ...
    file_cache.insert(module_path.canonicalize().unwrap().to_str().unwrap().into(), (config_yml.clone(), imports.clone(), main.clone()));
    ...
    modules.entry((config_yml.module_name, config_yml.version.clone())).or_insert(ModuleInfo { 
        path: module_path.canonicalize().unwrap().to_str().unwrap().into(), 
...
pub fn precompile_nessa_module_with_config(path: &String, all_modules: HashMap<(String, String), ModuleInfo>, file_cache: FileCache) -> Result<(NessaContext, Vec<NessaExpr>), NessaError> {
    let mut module = parse_nessa_module_with_config(&Path::new(path).canonicalize().unwrap().to_str().unwrap().into(), &mut HashMap::new(), &all_modules, &file_cache)?;
...
pub fn compute_project_hash(path: &String) -> Result<(String, HashMap<(String, String), ModuleInfo>, FileCache), NessaError> {
    ...
    let config_yml = &file_cache.get(module_path.canonicalize().unwrap().to_str().unwrap()).unwrap().0;

    let mut final_hash = config_yml.hash.clone();

    // Add the hashes of all submodules
    for (_, info) in &config_yml.modules {
        final_hash = format!("{}{}", final_hash, file_cache.get(Path::new(&info.path).canonicalize().unwrap().to_str().unwrap()).unwrap().0.hash);
    }
